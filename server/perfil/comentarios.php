...
try {
    ...
    $stmt3 = $pdo->prepare("SELECT contratos_concluidos, ultima_contratacao 
                            FROM usuarios WHERE id = ?");
    $stmt3->execute([$id_usuario]);
    $user = $stmt3->fetch(PDO::FETCH_ASSOC);

    // 4) Retorna o comentário recém criado
    $stmt4 = $pdo->prepare("SELECT id, id_usuario, nome, comentario, nota,
                            DATE_FORMAT(data_comentario, '%d/%m/%Y %H:%i:%s') AS data_comentario
    ...
    echo json_encode([
        ...
        'ultima_contratacao' => isset($user['ultima_contratacao']) 
            ? date('d/m/Y', strtotime($user['ultima_contratacao']))
            : null
